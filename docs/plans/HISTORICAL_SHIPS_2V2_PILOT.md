...
Do not invent balancing bonuses or fictional ship statistics. If the first ship on one side is materially stronger than its counterpart, compensate at the force-composition level by selecting the second historical ship so the two-ship forces are as comparable as reasonably possible.
...
2. Balance the pair composition by the choice of the second vessel on each side.
3. Use scenario geometry, wind and starting position only when historically/plausibly justified and explicitly documented.
4. Record residual asymmetry.
5. Later, once the enhanced simulation engine exists, validate balance empirically through repeated AI-vs-AI benchmark runs.
